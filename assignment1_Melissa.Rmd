...
specific_subjects_summary <- merged_data %>%
...
View(specific_subjects_summary)
...
#this follows the formula of the assignment where n1 = n2
  pooled_sd <- (sqrt(((n-1)*var(x1)) + ((n-1)*var(x2))  )) / n * 2 - 2
...
The researcher carried out their analysis on non-aggregrated data, by taking each datapoint per participant on each condition, this gives the degrees of freedom of (20 * 30 - 1) * 2  = 1198. This is not justified, because now the data is dependent, because participants saw both the words and the pseudowords. If the data is dependent, a t-test should not be used (or the dependancy in the data should be removed by aggregrating by subject for instance).  
...
A potential research question could be: What is the effect of the position of the body while taking an IQ test?
Then we have two conditions, one group would be standing while performing the IQ test and the other group would be sitting while taking the IQ test. Here we have the position of the body as the independent variable (standing/sitting) and the IQ test outcome as the dependent variable. Because IQ already normally distributed to begin with, the outcome will also be identically normally distributed in both conditions. Because we only let the participants take the test in one condition, the data is independent.
