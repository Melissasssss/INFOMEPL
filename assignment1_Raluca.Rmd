...
# This data frame stores information about items
str(itemdata)
head(itemdata)
...
# Summary for each subject separately

# merged_data <- left_join(responsedata, itemdata)
merged_data <- merge(responsedata, itemdata, by = "Item")
merged_data

# summarise not used anymore , group by does not work w reframe, replaced by .by
summary_whole <- merged_data %>%
 # group_by(IsWord) %>%
  summarise(
    mean_RT = mean(RT, na.rm = TRUE),
    sd_RT = sd(RT, na.rm = TRUE),
    range_RT = range(RT, na.rm = TRUE),
    mean_FreqCOCAspok = mean(FreqCOCAspok, na.rm = TRUE),
    sd_FreqCOCAspok = sd(FreqCOCAspok, na.rm = TRUE),
   # .by = IsWord
  )
summary_whole
...
$$s=\sqrt{\frac{(n-1)\cdot (var(x_1) +  var(x_2))}{2 \cdot n-2}}$$
...
$$SE=\sqrt{\frac{var(x_1) + var(x_2)}{n}}$$
...
print(paste("T value d for subject", subject_number, "is", t_value, "."))
...
print(paste("T value d for all words is", t_value, "."))
...
print(paste("T value d for specified vectors is", t_value, "."))
