...
# 1
# Fit linear regression models using each frequency source individually
model_FreqCOCA <- lm(log(RT) ~ FreqCOCA,merged_data)
model_FreqGoogle <- lm(log(RT) ~ FreqGoogle, data = merged_data)
model_FreqSUBTLEX <- lm(log(RT) ~ FreqSUBTLEX, data = merged_data)
model_FreqCOCAspok <- lm(log(RT) ~ FreqCOCAspok, data = merged_data)
...
ggplot(merged_data, aes(x = log_FreqGoogle, y = log(RT))) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Log FreqGoogle", y = "Log-transformed RT") +
  ggtitle("Relation between log RT and Log-transformed FreqGoogle")

# 3
# this part doesn't work yet
# Fit linear regression models using each frequency source individually
model_FreqCOCA <- lm(log(RT) ~ log(FreqCOCA),merged_data)
model_FreqGoogle <- lm(log(RT) ~ log(FreqGoogle), data = merged_data)
model_FreqSUBTLEX <- lm(log(RT) ~ log(FreqSUBTLEX), data = merged_data)
model_FreqCOCAspok <- lm(log(RT) ~ log(FreqCOCAspok), data = merged_data)
