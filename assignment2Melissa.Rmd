...
This is not a worry, because the verb region and spillover region are different parts of the dataset. It only becomes a problem if multiple tests are done on the same dataset, because when you do multiple tests on one single dataset you increase the chance of a false positive error (type 1 error).


There is a worry, because when you do multiple tests on one single dataset you increase the chance of a false positive error (type 1 error). This is because with more tests, you can by chance get a significant result, this chance gets higher for the more tests that you perform on that dataset. If you do 1000 tests and still use the 0.05 significance cutoff, you still can get 20 significant results just by chance. 
...
#ordinal <- for (i in dataStimMale) {ifelse(dataStimMale == 1| dataStimMale == 2,0, ifelse(dataStimMale == 4| dataStimMale == 5), 1, NaN)}

dataStimMaleLM <- selfies$Boring
BoringYesNo <-ifelse(dataStimMaleLM %in% c(1, 2), 0, ifelse(dataStimMaleLM %in% c(4, 5), 1, NaN)) 
