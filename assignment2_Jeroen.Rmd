...
The experiment (Experiment 1) described in the paper studies the effect of interference on memory retrieval. Imagine you are trying to replicate the experiment. For such a task, you need to gather all possible information about the setup of the experiment in the paper. Describe the experiment in enough details to make sure you would be able to replicate it as closely as possible. Find answers to at least these questions:
  ...
  Answer: The design was a 2x2 factorial arrangement with plausibility (plausible/implausible sentences) and a distractor (plausible/implausible distractors) as factors. They presented the experiment in a Latin square design to ensure all participants were exposed to all conditions in a balanced way. 
  2. Are the stimuli available? If so, where?
  Answer: The stimuli ??
  3. How many participants took part and how many items were used? There is an initial set of 40 items, later refined to 32 based on pre-tests for plausibility
  Answer: 48 native english speaking participants from the University of Reading were involved 
  ...
  Answer: The experiment collected eye-movement data, focusing on first pass times, regression path times and total viewing times at important regions of the texts. These measures  captured early and late processing efforts related to plausibility and interference effects.
  5. What statistical analyses were conducted? (Do not go into details here, name the statistical technique or tests used.)
  Answer: The statistical analyses used linear mixed-effects models with log-transformed reading times. The fixed effects where for plausibility, distractor, and region, along with their interactions.
...
Answer:
1. The first version involves individual reading times, which could be more affected by outliers and individual differences. The second, aggregated version, might better capture overall trends by reducing noise, but it also reduces the dataset size, potentially affecting power. Both approaches have their downsides and can be justified depending on the question
2. Testing the same hypothesis on different regions (verb and spillover) increases the risk of Type I errors. The Bonferroni correction is a method to adjust significance levels when making multiple comparisons to control the family-wise error rate. We need to consider applying this correction when interpreting p-values from multiple tests.

```{r}

# Craete verb data using filter for region and first-pass
verb_data <- exp1 %>%
  filter(region == "verb", measure == "fp")

# T-test for plausible vs. implausible in the verb region
t_test_verb <- t.test(rt ~ plausibility, data = verb_data, paired = FALSE)
print(t_test_verb)

# Unpaired t-test, is this right? Paired gives error so

# Craete spillover data using filter for region and first-pass
spillover_data <- exp1 %>%
  filter(region == "spillover", measure == "fp")

# T-test for plausible vs. implausible in the spillover region
t_test_spillover <- t.test(rt ~ plausibility, data = spillover_data, paired = FALSE)
print(t_test_spillover)

# Model for the verb region
lm_verb <- lm(rt ~ plausibility + region, data = exp1[exp1$measure == "fp", ])
summary(lm_verb)

# Model for the spillover region
lm_spillover <- lm(rt ~ plausibility + region, data = exp1[exp1$measure == "fp", ])
summary(lm_spillover)

#exp1_aggregated <- exp1 %>%
#  group_by(subject, plausibility, region) %>%
#  summarize(agg_rt = mean(rt))

exp1_aggregated <- exp1 %>%
  group_by(subject, plausibility, region) %>%
  summarize(agg_rt = mean(rt, na.rm = TRUE), .groups = "drop")

# Model for verb region
lm_agg_verb <- lm(agg_rt ~ plausibility + region, data = exp1_aggregated)
summary(lm_agg_verb)

# Model for spillover region
lm_agg_spillover <- lm(agg_rt ~ plausibility + region, data = exp1_aggregated)
summary(lm_agg_spillover)
...
model_a <- lmer(rt_log ~ plausibility * interference + (1|subject) + (1|item), 
