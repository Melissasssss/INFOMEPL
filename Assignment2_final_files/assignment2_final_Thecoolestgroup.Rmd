...
```
Within the t test we used a two sided paired t test, and we saw that with the means, the female stimulus had a higher boring score than the male stimulus. The t test confirmed it is a significant difference.
Within the generalized mixed linear model, we found that the male stimulus corresponded to a lower boring score, and this result was significant. Thus both methods give the same conclusion on the significance of Stimgender on boringness. 
```
...
#inspect the data
table(selfies["Gender"])
table(selfies["StimGender"])
...
#baseline model (all have cluster variable of participant)
...
#fixed stimgender, age and gender, only participant random
model_boring4 <- glmer(BoringYesNo ~ StimGender + Age + Gender + (1 | ResponseId), data = selfies, family = binomial(link = "logit"))

summary(model_boring4)
#Maximal model: fixed Stimgender, age and gender, random stimgender and participant, more complicated models did not converge.
model_boring5 <- glmer(BoringYesNo ~ StimGender + Age + Gender + (1 + StimGender| ResponseId), data = selfies, family = binomial(link = "logit"))


summary(model_boring5)
...
Aggregating the data and transforming the Boring resonses in binary form may lead to oversimplification, ignoring nuances in perception.
Counfounds are not taken into consideration such as lightning, background, context of selfie, the confounds may not even be clear exactly.
The confounds that are being considered are not controlled as well as they should for the results to be conclusive.
Within the sample there are almost 150 more male participants than female, thus there is an imbalance within the data. Imbalanced data can lead to conclusions that may seem conclusive between gender and perceived boredom from the selfies, but in reality the imbalance may lead to bias and therefore to inconclusive results. It could be that because of the skew in data, the conclusion should not be that female selfies are less boring, it could be that male persons find female selfies more boring than male selfies. 
...
#model <- glmer(BoringYesNo ~ 1 + StimGender*Gender + (1 + StimGender * Gender | ResponseId), data = selfies, family = binomial(link = "logit"))

# Fit the mixed-effects logistic regression model
model <- glmer(BoringYesNo ~ StimGender * Gender + (1 | StimGender), data = selfies, family = binomial(link = "logit"))
...
```{r}
# Get all unique combinations of StimGender and Gender
combinations <- expand.grid(StimGender = unique(novel_selfies$StimGender), Gender = unique(novel_selfies$Gender))

# Predict probabilities for each combination
combinations$predicted_prob <- predict(model, newdata = combinations, type = "response")

# View the predicted probabilities for each combination
print(combinations)
...
# Predict probabilities for all rows in the novel data
novel_selfies$predicted_prob <- predict(model, newdata = novel_selfies, type = "response")

mypredictions <- novel_selfies$predicted_prob

# View the novel data with predicted probabilities
print(novel_selfies[, c("StimGender", "Gender", "predicted_prob")])
...
```
For each assignment, we split up the exercises between ourselves and checked at the labs the results. We checked everything during the assignment and in the end combined all answeres. This worked well for our group.
